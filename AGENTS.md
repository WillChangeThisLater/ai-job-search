...
- Resume versioning: `resumes/<field>/wendt_paul_resume.md` is the **single copy** per field — no `_v1`/`_v2` snapshot files in `resumes/`. When the human approves a resume for submission, copy the rendered PDF into the application directory as `applications/<application>/wendt_paul_resume.pdf`, and point the application's `resume_used:` frontmatter at that copy, so each application records exactly which resume was submitted while `resumes/` stays one-source-of-truth.
